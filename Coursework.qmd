...
Prior to conducting the analysis of credit risk, we need to understand and organise the data. For this analysis we will be using a loan defaulting dataset from Kaggle (reference), consisting of 12 variables/columns and 32,581 observations, illustrated in Table 1.
...
Figure 7 visualises the error within the classification model. The matrix reveals that the model correctly identified 1,083 non-default cases (true negatives) and 1,093 default cases (true positives), demonstrating its ability to capture both classes effectively. However, 337 non-default cases were misclassified as defaults (false positives), while 323 default cases were incorrectly predicted as non-defaults (false negatives), potentially leading to losses in revenue for a financial institution.
