...
TN = conf_df_rf.iloc[0, 0]  # True Negatives (No Default correctly classified)
FP = conf_df_rf.iloc[0, 1]  # False Positives (Non-default misclassified as Default)
FN = conf_df_rf.iloc[1, 0]  # False Negatives (Default misclassified as Non-default)
TP = conf_df_rf.iloc[1, 1]  # True Positives (Default correctly classified)
```

The confusion matrix (Figure 10) for the RF model provides a detailed comparison of actual versus predicted default status. In this case, the model correctly predicted Non-default for `{python} str(TN)` instances (True Negatives), and correctly identified "Defaulting" for `{python} str(TP)` instances (True Positives). However, there were `{python} str(FP)` False Positives, where the model incorrectly predicted "Defaulting" when the actual class was "No default," and `{python} str(FN)` False Negatives. This confusion matrix reiterates the improved performance from the LR as the incorret classification instances have decreased.
...
plt.xlabel("False Positive Rate (1 - Specificity)")
plt.ylabel("True Positive Rate (Sensitivity)")
plt.suptitle("Figure ?: ROC Curve for XGBoost Model", y = -0.05, fontsize=9)
plt.legend()
plt.show()
```
...
plt.xlabel("Predicted Class")
plt.ylabel("Actual Class")
plt.suptitle("Figure ?: Confusion Matrix for XGBoost Model", y = -0.05, fontsize=9)
plt.show()

TN = conf_df.iloc[0, 0]  # True Negatives (No Default correctly classified)
FP = conf_df.iloc[0, 1]  # False Positives (Non-default misclassified as Default)
FN = conf_df.iloc[1, 0]  # False Negatives (Default misclassified as Non-default)
TP = conf_df.iloc[1, 1]  # True Positives (Default correctly classified)
```
...
plt.xlabel('Importance')
plt.ylabel('Feature')
plt.suptitle("Figure ?: Feature Importances from XGBoost Model", y = -0.05, fontsize=10, x = 0.5)
plt.show()
...
#imp_Feature_1 = importance_df_xgb[Feature_1].round(3)
#imp_Feature_2 = importance_df_xgb[Feature_2].round(3)
#imp_Feature_3 = importance_df_xgb[Feature_3].round(3)
```
...
plt.xlabel("False Positive Rate (1 - Specificity)")
plt.ylabel("True Positive Rate (Sensitivity)")
plt.suptitle("Figure ?: ROC Curve for LightGBM Model", y = -0.05, fontsize=9)
plt.legend()
plt.show()
```
...
plt.xlabel("Predicted Class")
plt.ylabel("Actual Class")
plt.suptitle("Figure ?: Confusion Matrix for LightGBM Model", y = -0.05, fontsize=9)
plt.show()

TN = conf_df_lgbm.iloc[0, 0]  # True Negatives (No Default correctly classified)
FP = conf_df_lgbm.iloc[0, 1]  # False Positives (Non-default misclassified as Default)
FN = conf_df_lgbm.iloc[1, 0]  # False Negatives (Default misclassified as Non-default)
TP = conf_df_lgbm.iloc[1, 1]  # True Positives (Default correctly classified)
...
plt.figure(figsize=(8, 5))
sns.barplot(x='Importance', y='Feature', data=importance_df_lgbm, palette=sns.color_palette("Greys", n_colors=len(importance_df_lgbm)), edgecolor='black')
plt.xlabel('Importance')
plt.ylabel('Feature')
plt.suptitle("Figure ?: Feature Importances from LightGBM Model", y = -0.05, fontsize=10, x = 0.5)
plt.show()

top_features_lgbm = importance_df_lgbm.nlargest(3, 'Importance')['Feature'].tolist()  # Random Forest
Feature_1 = top_features_lr[2]
Feature_2 = top_features_lr[1]
Feature_3 = top_features_lr[0]

#imp_Feature_1 = importance_df_lgbm[Feature_1].round(3)
#imp_Feature_2 = importance_df_lgbm[Feature_2].round(3)
#imp_Feature_3 = importance_df_lgbm[Feature_3].round(3)
```
...
top_features_lgbm = importance_df_lgbm.nlargest(3, 'Importance')['Feature'].tolist()  # LightGBM
