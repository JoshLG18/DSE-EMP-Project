...
grid_search = GridSearchCV(GaussianNB(), param_grid, cv=5, scoring='accuracy', verbose=0, n_jobs=-1)
grid_search.fit(X_train, y_train)
...
plt.figure(figsize=(8, 5))
plt.plot(fpr_nb, tpr_nb, color="black", linewidth=2, label=f"AUC: {auc_value_NB}")
plt.plot([0, 1], [0, 1], linestyle="--", color="grey")  # Reference diagonal
plt.xlabel("False Positive Rate (1 - Specificity)")
plt.ylabel("True Positive Rate (Sensitivity)")
...
# Extract coefficients (mean values for each feature per class)
coefficients = best_nb_model.theta_[1] - best_nb_model.theta_[0]
features = X_train.columns
...
# Plot the coefficients in black and white with black outlines
plt.figure(figsize=(10, 6))
sns.barplot(x='Coefficient', y='Feature', data=coefficients_df, palette=sns.color_palette("Greys", n_colors=len(coefficients_df)), edgecolor='black')
plt.xlabel('Coefficient')
plt.ylabel('Feature')
plt.suptitle("Figure ?: Coefficients from Gaussian Naive Bayes Model", y = 0.0005, fontsize=11)
plt.show()
```
...
accuracy_df = pd.DataFrame({
    "Model": ["RF", "NB", "XGB"],
    "Value": [accuracyRF, accuracyNB, accuracyXGB]
})
...
performance_df = pd.DataFrame({
    "Model": ["RF", "NB", "XGB"],
    "Accuracy": [accuracyRF, accuracyNB, accuracyXGB],
    "Precision": [precisionRF, precisionNB, precisionXGB],
    "Recall": [recallRF, recallNB, recallXGB],
    "F1_Score": [f1_scoreRF, f1_scoreNB, f1_scoreXGB],
    "AUC": [auc_value_RF, auc_value_NB, auc_value_XGB]
})
