...
However, there are trade-offs. These models are harder to interpret, which can be a challenge in regulated sectors where transparency is important (Afolabi, 2024). They also demand more computational power, which might limit access for smaller institutions [@Lei:2025]. Without proper tuning, they can overfit on skewed data [@Cheng:2021]. There's also the risk of reinforcing biases present in the training data, potentially leading to unfair outcomes in lending [@Shah:2025]. Moving forward, improving explainability, tuning, and fairness will be essential for making these models more practical and trustworthy.
